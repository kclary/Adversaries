...
int print_network(deque<set<int> > & E, const deque<deque<int> > & member_list, const deque<deque<int> > & member_matrix, deque<int> & num_seq, string netfile, string comfile) {
...
int benchmark(bool excess, bool defect, int num_nodes, double  average_k, int  max_degree, double  tau, double  tau2, 
	...
	print_network(E, member_list, member_matrix, num_seq, netfile, comfile);
...
int main(int argc, char * argv[]) {
	...
	erase_file_if_exists(p.netfile); 
    erase_file_if_exists(p.comfile); 
	//erase_file_if_exists("statistics.dat");
	
	benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree, p.tau, p.tau2, p.mixing_parameter, p.overlapping_nodes, p.overlap_membership, p.nmin, p.nmax, p.fixed_range, p.clustering_coeff, p.netfile, p.comfile);	
